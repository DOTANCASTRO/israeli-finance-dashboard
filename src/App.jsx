...
export default function App() {
  ...
  const processFile = useCallback((file) => {
    setError(""); setAiResponse(""); setFileName(file.name);
    ...
    reader.onload = (e) => {
      try {
        ...
        if (txs.length === 0) throw new Error("No transactions detected. Check the file format.");
        setTransactions(txs);
      } catch (err) {
  ...
  const totalCredit = useMemo(() => transactions.filter(t => t.isCredit).reduce((s,t)=>s+t.amount,0), [transactions]);
      ...
      <div className="app">
        ...
        {transactions.length === 0 ? (
          <div
            ...
            onDrop={onDrop}
          >
            <input type="file" accept=".csv,.xlsx,.xls" onChange={onFileInput} />
          ...
          <>
            ...
            <div style={{ marginTop: "1rem", textAlign: "right" }}>
              <button onClick={() => { setTransactions([]); setAiResponse(""); setFileName(""); }}
                style={{ background: "none", border: "1px solid rgba(255,255,255,0.1)", color: "#6b7280", padding: "0.5rem 1rem", borderRadius: 6, cursor: "pointer", fontFamily: "DM Mono", fontSize: "0.8rem" }}>
                ← Load different file
              </button>
            </div>
          </>
